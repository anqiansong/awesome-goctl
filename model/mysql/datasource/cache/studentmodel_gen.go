...
var (
	...
	cacheTestStudentTypePrefix = "cache:test:student:type:"
)
...
func newStudentModel(conn sqlx.SqlConn, c cache.CacheConf) *defaultStudentModel {
...
func (m *defaultStudentModel) Delete(ctx context.Context, tp int64) error {
	testStudentTypeKey := fmt.Sprintf("%s%v", cacheTestStudentTypePrefix, tp)
...
func (m *defaultStudentModel) FindOne(ctx context.Context, tp int64) (*Student, error) {
	testStudentTypeKey := fmt.Sprintf("%s%v", cacheTestStudentTypePrefix, tp)
	var resp Student
	err := m.QueryRowCtx(ctx, &resp, testStudentTypeKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
func (m *defaultStudentModel) Insert(ctx context.Context, data *Student) (sql.Result, error) {
	testStudentTypeKey := fmt.Sprintf("%s%v", cacheTestStudentTypePrefix, data.Type)
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?, ?, ?, ?)", m.table, studentRowsExpectAutoSet)
		return conn.ExecCtx(ctx, query, data.Type, data.Class, data.Name, data.Age, data.Score)
	}, testStudentTypeKey)
	return ret, err
}
...
func (m *defaultStudentModel) Update(ctx context.Context, data *Student) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Class, data.Name, data.Age, data.Score, data.Type)
	}, testStudentTypeKey)
	return err
}

func (m *defaultStudentModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheTestStudentTypePrefix, primary)
}

func (m *defaultStudentModel) queryPrimary(ctx context.Context, conn sqlx.SqlConn, v, primary interface{}) error {
	query := fmt.Sprintf("select %s from %s where `type` = ? limit 1", studentRows, m.table)
